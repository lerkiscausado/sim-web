...
import { Search, Plus, Loader2, Pencil, Ban, CheckCircle2, GraduationCap } from "lucide-react";
...
export default function Especialidades() {
    ...
    const [editando, setEditando] = useState<EspecialidadItem | null>(null);
    ...
    const cargar = useCallback(async (p: number, q?: string) => {
        ...
        try {
            ...
            if (q) qs.set("q", q);
            const data = await api.get<Paginado>(`/catalogos/especialidades?${qs.toString()}`);
            setResult(data);
        } catch (err) {
            setError(err instanceof ApiError ? err.message : "No se pudo cargar el catálogo de especialidades");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    function abrirEditar(item: EspecialidadItem) {
        setEditando(item);
        setForm({ nombreEspecialidad: item.nombreEspecialidad });
        setFormError(null);
        setDialogOpen(true);
    }
    ...
    async function guardar() {
        ...
        try {
            if (editando) {
                await api.patch(`/catalogos/especialidades/${editando.id}`, form);
            } else {
                await api.post("/catalogos/especialidades", form);
            }
            setDialogOpen(false);
    ...
    async function toggleEstado(item: EspecialidadItem) {
        const nuevo = item.estado === "ACTIVO" ? "INACTIVO" : "ACTIVO";
        try {
            await api.patch(`/catalogos/especialidades/${item.id}/estado/${nuevo}`);
            await cargar(page, searchTerm);
        } catch (err) {
        ...
        <Card className="border-none shadow-none bg-transparent">
            <CardHeader className="px-0 pt-0">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <CardTitle className="text-xl font-bold">Catálogo de Especialidades</CardTitle>
                        <CardDescription>Gestión de las especialidades médicas del cuerpo asistencial.</CardDescription>
                    </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-muted/50">
                                <TableHead className="font-bold"><span className="inline-flex items-center gap-1.5"><GraduationCap className="h-3.5 w-3.5" />Nombre</span></TableHead>
                        ...
                        <TableBody>
                            ...
                            {result?.data.map((item) => (
                                <TableRow key={item.id} className="hover:bg-muted/30 transition-colors">
                                    <TableCell className="font-semibold">{item.nombreEspecialidad}</TableCell>
