...
import { Search, Plus, Loader2, Pencil, Ban, CheckCircle2, Briefcase } from "lucide-react";
...
export default function Cargos() {
    ...
    const [editando, setEditando] = useState<CargoItem | null>(null);
    const [form, setForm] = useState({ nombreCargo: "" });
    ...
    const cargar = useCallback(async (p: number, q?: string) => {
        ...
        try {
            ...
            if (q) qs.set("q", q);
            const data = await api.get<Paginado>(`/catalogos/cargos?${qs.toString()}`);
            setResult(data);
        } catch (err) {
            setError(err instanceof ApiError ? err.message : "No se pudo cargar el catálogo de cargos");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    function abrirEditar(item: CargoItem) {
        setEditando(item);
        setForm({ nombreCargo: item.nombreCargo });
        setFormError(null);
        setDialogOpen(true);
    }
    ...
    async function guardar() {
        ...
        try {
            if (editando) {
                await api.patch(`/catalogos/cargos/${editando.id}`, form);
            } else {
                await api.post("/catalogos/cargos", form);
            }
            setDialogOpen(false);
    ...
    async function toggleEstado(item: CargoItem) {
        const nuevo = item.estado === "A" ? "I" : "A";
        try {
            await api.patch(`/catalogos/cargos/${item.id}/estado/${nuevo}`);
            await cargar(page, searchTerm);
        } catch (err) {
            ...
            <CardHeader className="px-0 pt-0">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <CardDescription>Gestión de los cargos del personal.</CardDescription>
                    </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-muted/50">
                                <TableHead className="font-bold"><span className="inline-flex items-center gap-1.5"><Briefcase className="h-3.5 w-3.5" />Nombre</span></TableHead>
                        ...
                        <TableBody>
                            ...
                            {result?.data.map((item) => (
                                <TableRow key={item.id} className="hover:bg-muted/30 transition-colors">
                                    <TableCell className="font-semibold">{item.nombreCargo}</TableCell>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>{editando ? "Actualiza el nombre del cargo." : "Registra un nuevo cargo del personal."}</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-3 py-2">
                        <div className="space-y-1.5">
                            ...
                            <Input
                                ...
                                onChange={(e) => setForm({ nombreCargo: e.target.value })}
                                maxLength={50}
                            />
                        </div>
